...
p9 <- dat %>% mutate(traveltime = ifelse(fedu=="1", "<15min", ifelse(fedu==2, "15 a 30min", ifelse(fedu==3, "30 a 60min", "> 60min")))) %>% ggplot(aes(x = traveltime, y = g3)) + geom_boxplot() + theme_minimal() + labs(title = paste("Tempo de Viagem vs", " G3"), x = "Tempo de Viagem", y = y_name)
p9 %>% ggsave(filename = "img/tempo_viagem.png", dpi = "retina", width = w, height = h)

p10 <- dat %>% mutate(studytime = ifelse(fedu=="1", "<2h", ifelse(fedu==2, "2 a 5h", ifelse(fedu==3, "5 a 10h", "> 10h")))) %>% ggplot(aes(x = studytime, y = g3)) + geom_boxplot() + theme_minimal() + labs(title = paste("Tempo de Estudo vs", " G3"), x = "Tempo de Estudo", y = y_name)
...
p17 <- dat %>% mutate(freetime = health %>% as.character()) %>% ggplot(aes(x = freetime , y = g3)) + geom_boxplot() + theme_minimal() + labs(title = paste("Tempo Livre vs", " G3"), x = "Tempo Livre", y = y_name)
...
dat %<>% 
  mutate(
    ...
    #traveltime = ifelse(fedu=="1", "<15min", ifelse(fedu==2, "15 a 30min", ifelse(fedu==3, "30 a 60min", "> 60min"))),
    #studytime = ifelse(fedu=="1", "<2h", ifelse(fedu==2, "2 a 5h", ifelse(fedu==3, "5 a 10h", "> 10h"))),
